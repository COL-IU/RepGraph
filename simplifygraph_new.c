...
char	inpfile[100], graphfile[100], edgefile[100], outfile[100], seqfile[100], intvfile[100];
	...
	do	{
		...
		for(i = 0; i < num_vertex; i ++)	{
			...
			while(j < vertex[i] -> num_nextedge)	{
				...
				if(edge1 -> begin == edge1 -> end && edge1 -> length < TANDEM_LEG)	{
...
/*	Find important edges, i.e. multiplicity >= min_multip; Length >= min_length	*/
	...
	for(i = 0; i < num_vertex; i ++)	{
		for(j = 0; j < vertex[i] -> num_nextedge; j ++)	{
			edge1 = vertex[i] -> nextedge[j];
			if(edge1 -> visit == 0 && (edge1 -> multip >= min_multip2 ||
			  (edge1 -> multip >= min_multip && edge1 -> length >= min_length)))	{
...
/*	Output read intervals in each edge	*/
	sprintf(temp, "%s.intv.new", inpfile);
	fp = ckopen(temp, "w");
...
void initenv(int argc, char **argv)
{
	...
	while ((copt=getopt(argc,argv,"i:o:e:g:v:m:L:M:t:")) != EOF)	{
		switch(copt) {
			case 'i':
			  inpseq = 1;
			...
			case 'M':
			  ...
			  continue;
			default:
			  printf("simplifygraph_new -i InpFile -o outfile [-e edgefile -g graphfile -v intvfile -M min_multiplicity -L minimal_edge_length]\n");
	...
	if(inpseq == 0 || outseq == 0)	{
		  printf("simplifygraph_new -i InpFile -o outfile [-e edgefile -g graphfile -v intvfile -mM min_multiplicity -L minimal_edge_length]\n");
		  printf("-i InpFile: The input file name of sequences\n");
		  printf("-o Outfile: The output file name of graphs\n");
		  printf("-e edgefile (optional): default using inpfile.edge\n");
		  printf("-g graphfile (optional): default using inpfile.graph\n");
